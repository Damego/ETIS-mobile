...
export default class DataParsing {
    ...
    parseHeader(html) {
        ...
        // Сленг ВУЗа
        let header_caption = $('body > div.navbar.navbar-static-top > div > div > a').text();
        ...
        let header_speciality = $('body > div.navbar.navbar-static-top > div > div > div > div > span > span:nth-child(1)').text();
        data['speciality'] = header_speciality;

        // Форма обчуения
        let header_form_of_education = $('body > div.navbar.navbar-static-top > div > div > div > div > span > span:nth-child(2)').text();
        data['form_of_education'] = header_form_of_education;

        // Текущий год обучения
        let header_year = $('body > div.navbar.navbar-static-top > div > div > div > div > span > span:nth-child(3)').text();
